...
data class NoteRow(@PrimaryKey val id: String, val revision: Long,
    val title: String, val text: String, val updatedAt: Long)

@Entity(tableName = "note_revisions", primaryKeys = ["noteId", "revision"])
data class NoteRevisionRow(val noteId: String, val revision: Long,
    val title: String, val text: String, val committedAt: Long)

@Entity(tableName = "command_receipts")
data class ReceiptRow(@PrimaryKey val commandId: String, val noteId: String,
...
@Database(entities = [NoteRow::class, NoteRevisionRow::class, ReceiptRow::class,
    InkPageRow::class, InkStrokeRow::class, InkReceiptRow::class],
        ...
        fun open(context: Context, name: String = "inkweft-a0.db"): NoteDatabase =
            Room.databaseBuilder(context.applicationContext, NoteDatabase::class.java, name)
                ...
                .addCallback(object : Callback() {
                    override fun onOpen(db: SupportSQLiteDatabase) { db.execSQL("PRAGMA synchronous=FULL") }
                })
                // Additive v1->v2 migration. Never use destructive fallback or corruption deletion.
                .build()
    }
}
