...
interface NotebookPageDao {
    @Query("SELECT * FROM notebook_pages WHERE notebookId=:id ORDER BY position,id") fun observe(id:String):Flow<List<NotebookPageRow>>
    @Query("SELECT * FROM notebook_pages WHERE notebookId=:id ORDER BY position,id") suspend fun list(id:String):List<NotebookPageRow>
    ...
    @Query("UPDATE notebook_pages SET position=position+1 WHERE notebookId=:id AND position>:after") suspend fun shiftAfter(id:String,after:Int)
    ...
    @Query("SELECT p.notebookId AS notebookId,p.id AS pageId,p.position AS position,s.text AS text FROM page_search_text s JOIN notebook_pages p ON p.id=s.pageId LEFT JOIN ink_pages h ON h.noteId=p.id WHERE s.inkRevision=COALESCE(h.revision,0)")
...
class NotebookPages(private val db:NoteDatabase) {
    ...
    suspend fun addAfter(notebookId:String,afterId:String,newId:String):NotebookPageRow=db.withTransaction {
        ...
        val after=checkNotNull(db.pages().get(afterId));require(after.notebookId==notebookId&&!after.world)
        ...
        require(db.pages().list(notebookId).size<500){"Page budget reached"}
    ...
    suspend fun select(notebookId:String,pageId:String)=db.withTransaction {
        val p=checkNotNull(db.pages().get(pageId));require(p.notebookId==notebookId)
    ...
    suspend fun saveSearchText(pageId:String,expectedInkRevision:Long,text:String):Boolean=db.withTransaction {
        checkNotNull(db.pages().get(pageId));require(text.length<=20_000)
