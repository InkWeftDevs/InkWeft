...
class InkPageFile(val title:String,val text:String,strokes:List<InkStroke>,val world:Boolean=false,val paper:PaperStyle=PaperStyle.RULED,objects:List<PageObject> = emptyList()){
    val objects:List<PageObject> = Collections.unmodifiableList(ArrayList(objects))
    val strokes:List<InkStroke> = Collections.unmodifiableList(ArrayList(strokes))
    init{PageObjectCodec.encode(objects);require(title.isNotBlank()&&title.length<=120&&text.length<=100_000);require(strokes.size<=InkLimits.MAX_STROKES&&strokes.map{it.id}.distinct().size==strokes.size);require(strokes.sumOf{it.samples.size}<=InkLimits.MAX_PAGE_POINTS);require(strokes.all{it.world==world})}
    fun encode():ByteArray {
        val body=ByteArrayOutputStream();DataOutputStream(body).use{out->
            out.writeInt(if(objects.isNotEmpty())0x49575035 else if(strokes.any{s->s.cuts.any{it.shape!=InkCutShape.ROUND}})0x49575034 else if(strokes.any{it.cuts.isNotEmpty()})0x49575033 else 0x49575032);out.writeBoolean(world);out.writeByte(paper.ordinal)
            ...
            if(objects.isNotEmpty()){val encoded=PageObjectCodec.encode(objects);out.writeInt(encoded.size);out.write(encoded)}
    ...
    companion object {
        ...
        fun decode(bytes:ByteArray):InkPageFile {
            ...
            return DataInputStream(ByteArrayInputStream(body)).use{input->
                val magic=input.readInt();require(magic in listOf(0x49575031,0x49575032,0x49575033,0x49575034,0x49575035)){"Unknown page copy version"}
                ...
                require(input.available()==0);InkPageFile(title,text,strokes,world,paper,objects)
            }
        }
    }
}
