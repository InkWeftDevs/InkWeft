...
private fun BeautifyDialog(original:List<InkStroke>,dismiss:()->Unit,apply:(List<InkStroke>)->Unit){
    ...
    AlertDialog(onDismissRequest=dismiss,modifier=Modifier.testTag("beautify-dialog"),title={Text("稳线美化 · 先预览后应用")},text={Column(Modifier.verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(10.dp)){
        ...
        AndroidView(factory={InkCanvasView(it).apply{this.preview=true}},update={it.configure(true,PaperStyle.BLANK,null);it.showStrokes(if(comparison)original else preview?:original)},modifier=Modifier.fillMaxWidth().height(180.dp).testTag("beautify-preview"))
    ...
    }},confirmButton={TextButton(onClick={preview?.let(apply)},enabled=preview!=null,modifier=Modifier.testTag("apply-beautify")){Text("应用到选中笔迹")}},dismissButton={TextButton(onClick=dismiss){Text("取消")}})
}
