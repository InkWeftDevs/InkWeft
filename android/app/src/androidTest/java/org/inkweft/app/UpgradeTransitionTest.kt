...
class UpgradeTransitionTest {
    ...
    private val baseSchema get()=InstrumentationRegistry.getArguments().getString("baseSchema","7").toInt().also{require(it==4||it==7)}
    ...
    @Test fun seedBaseline(){
        assertEquals(if(baseSchema==4)5L else 9L,ctx.packageManager.getPackageInfo(ctx.packageName,0).longVersionCode)
        ...
        try{assertEquals(0,sql.version);sql.beginTransaction()
            ...
            sql.execSQL("INSERT INTO page_search_text VALUES(?,1,'升级后仍可查找','MANUAL')",arrayOf(book));sql.version=baseSchema
    ...
    @Test fun verifyAfterUpgrade(){
        ...
        println("UPGRADE_SCHEMA_${baseSchema}_TO_8_PRESERVED_DATA_NO_UNINSTALL")
    }
}
