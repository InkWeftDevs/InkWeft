...
class UpgradeTransitionTest {
    ...
    private val baseSchema get()=InstrumentationRegistry.getArguments().getString("baseSchema","7").toInt().also{require(it==4||it==7||it==8)}
    ...
    @Test fun seedBaseline(){
        assertEquals(if(baseSchema==4)5L else if(baseSchema==7)9L else 10L,ctx.packageManager.getPackageInfo(ctx.packageName,0).longVersionCode)
    ...
    @Test fun verifyAfterUpgrade(){
        assertEquals(11L,ctx.packageManager.getPackageInfo(ctx.packageName,0).longVersionCode)
        ...
        while(android.os.SystemClock.elapsedRealtime()<deadline&&!migrated){
            migrated=runCatching{SQLiteDatabase.openDatabase(ctx.getDatabasePath("inkweft-a0.db").absolutePath,null,SQLiteDatabase.OPEN_READONLY).use{it.version==9}}.getOrDefault(false)
        ...
        println("UPGRADE_SCHEMA_${baseSchema}_TO_9_PRESERVED_DATA_NO_UNINSTALL")
    }
}
