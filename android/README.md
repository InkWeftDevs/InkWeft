...
当前为 **A3.5 学习工作台测试版**（`0.0.10-a3.5-learning`）：框选／套索、局部擦除、稳线美化、手工摘要卡、大纲与基础思维导图。当前功能、最终验证与限制见 [修改日志](CHANGELOG.md)，开发顺序见 [待实现内容](TODO.md)，交互与数据合同见 [A3.5 说明](A3.5-LEARNING.md)。

以下内容保留各早期阶段的历史记录；其中“未完成”和测试数字描述的是当时状态，不代表 A3.5 的现状。
