...
当前为 **自定义封面测试版**（`0.0.12-custom-covers`）：支持本地图片封面、配色、文字和取景调整；在 UI-R2 基础上保留可视化模板创建、双层编辑工具、框选／稳线、共享摘要、知识关联、局部图、集合与手动回忆。当前实现与验证边界见 [UI-R2 实施记录](UI-R2-IMPLEMENTATION.md) 和 [修改日志](CHANGELOG.md)，开发顺序见 [待实现内容](TODO.md)，既有学习数据合同见 [A3.5 说明](A3.5-LEARNING.md)。
